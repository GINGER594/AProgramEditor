...
const settingsPath string = "/APESettings.cfg"

var validTypes []string = []string{"keyword", "symbols", "strings", "comment"}
...
func CreateInternalSettingsFileIfNotExists() error {
...
func parseSettingsEntry(line string) (string, string, string, string, error) {
    ...
    if len(tokens) != 4 {
        return "", "", "", "", fmt.Errorf("malformed entry: '%s'\r\nrun with the '-settings' flag to edit APESettings", line)
    }
...
//reads and parses syntax hue settings, returns settings as a HueMap, filtered by file extension
