...
const helpMessage string = "\r\n" +
    ...
    "\r\n" +
    "flags:\r\n" +
    "> NCT can be run with the flags '-help', '-settings' & '-d'\r\n" +
    "> running with the '-help' flag instead of a filepath will display a help message\r\n" +
    "> running with the '-settings' flag instead of a filepath will open syntax highlighting settings (without reading the settings file)\r\n" +
...
//returns path, file ext and whether or not the file is to be opened without syntax highlighting
func validateOsArgs() (string, string, bool, error) {
    if len(os.Args) != 2 && len(os.Args) != 3 {
        return "", "", false, fmt.Errorf("invalid no. of arguments recieved - epected: 1 or 2, got: %d", len(os.Args)-1)
    }
    ...
    if len(strings.Split(path, ".")) != 2 {
        return "", "", false, fmt.Errorf("invalid path argument: no file extension present")
    }
    ...
    if len(os.Args) == 3 {
        if os.Args[2] == debugFlag {
            ...
            return "", "", false, fmt.Errorf("invalid flag: '%s'", os.Args[2])
        }
    }
    return path, ext, noSettings, nil
...
func runTextEditor() error {
    //arg validation
    path, ext, noSettings, err := validateOsArgs()
    if err != nil {
